...
// Spatial Grid for O(n) complexity neighbor detection
class SpatialGrid {
  cellSize: number;
  cols: number;
  rows: number;
  cells: Map<string, NetworkNode[]>;
  ...
  getNearby(node: NetworkNode, radius: number): NetworkNode[] {
    const centerCol = Math.floor(node.x / this.cellSize);
    const centerRow = Math.floor(node.y / this.cellSize);
...
export const CanvasBackground = () => {
  ...
  // Update colors when theme changes (without resetting animation)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      // MASSIVELY INCREASED DENSITY with Spatial Grid optimization
      const area = width * height;
      let nodeCount;

      // With O(n) algorithm, we can handle 3-5x more nodes
      if (width > 1920 || height > 1080) {
        nodeCount = Math.min(Math.floor(area / 5000), 300);
      } else if (dpr > 1.5) {
        nodeCount = Math.min(Math.floor(area / 4000), 350);
      } else {
        nodeCount = Math.min(Math.floor(area / 3500), 400);
      }

      nodeCount = Math.max(nodeCount, 80); // Minimum 80 nodes (was 40)

      nodesRef.current = [];
      packetsRef.current = [];
      linksRef.current = [];
      lastTopologyUpdate.current = 0;
    ...
    const draw = (time: number) => {
      ...
      if (elapsed > frameInterval) {
        ...
        // Update Topology (Links) using Spatial Grid - O(n) complexity!
        if (time - lastTopologyUpdate.current > 300) {
          ...
          for (let i = 0; i < nodeCount; i++) {
            ...
            // Only check nearby nodes from grid - typically 9-25 cells
            const nearbyNodes = grid.getNearby(nodeA, maxDist);
            ...
            for (let j = 0; j < nearbyNodes.length; j++) {
              ...
              const pairId = nodeA.x < nodeB.x || (nodeA.x === nodeB.x && nodeA.y < nodeB.y)
              ...
              if (nodeBIndex === -1 || connectionCounts[nodeBIndex] >= maxConnections) continue;
        ...
        // Gestión de Packets - Optimizada y escalada con densidad de nodos
        const packets = packetsRef.current;

        // Remove inactive packets in one pass
        for (let i = packets.length - 1; i >= 0; i--) {
          const packet = packets[i];
          if (!packet.active) {
            const current = packet.to;

            // 25% chance to hop to next node (increased from 15%)
            if (current.neighbors.length > 0 && Math.random() < 0.25) {
        ...
        // Scale packet count with node density
        // More nodes = more traffic capacity
        const maxPackets = Math.min(nodeCount * 0.5, 200); // 0.5 packets per node, max 200
        const spawnRate = 0.35; // Increased from 0.25
        ...
        // Update y Draw Packets - Batched for performance
        ctx.globalAlpha = 1.0; // Reset from node drawing
      ...
      <GlassDock>
        {navItems.map((item) => (
          ...
          >
            ...
            <span className="absolute -top-14 opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-all duration-300 transform scale-75 group-hover:scale-100 group-focus:scale-100 px-3 py-1.5 rounded-xl bg-[var(--dock-bg)] backdrop-blur-xl border border-[var(--card-border)] text-[11px] font-semibold tracking-wide text-[var(--text-primary)] shadow-xl pointer-events-none whitespace-nowrap z-50">
              {item.label}
            </span>
          </a>
        ))}
